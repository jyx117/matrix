...
public class LoginInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response,
        ...
        if (StringUtil.isBlank(token)) {
            ...
            writeResponse(error(ErrorCode.BIZ_NOT_LOGIN), response);
        ...
        try {
            ...
            writeResponse(error(ErrorCode.BIZ_LOGIN_EXCEPTION), response);
        ...
        if (combines.length != TOKEN_SIZE) {
            writeResponse(error(ErrorCode.BIZ_TOKEN_INVALID), response);
        ...
        if (System.currentTimeMillis() >= time) {
            writeResponse(error(ErrorCode.BIZ_TOKEN_EXPIRED), response);
            throw new BizException(ErrorCode.BIZ_TOKEN_EXPIRED);
        }
        UserAuth userAuth = userAuthService.getByUserId(userId);
        // 用户不存在
        if (null == userAuth) {
            writeResponse(error(ErrorCode.BIZ_USER_NOT_EXIST), response);
        ...
        try {
            ...
            writeResponse(error(ErrorCode.BIZ_TOKEN_VERIFY_FAILED), response);
    ...
    private BaseResult error(ErrorCode errorCode) {
